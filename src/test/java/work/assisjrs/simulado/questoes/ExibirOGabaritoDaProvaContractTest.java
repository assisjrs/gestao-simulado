...
@DatabaseSetups({
        ...
        @DatabaseSetup("/datasets/questoes/ExibirOGabaritodaProvaContractTest.xml")
})
public class ExibirOGabaritoDaProvaContractTest {
    @Value("${local.server.port}")
    private int port = 0;

    @Test
    public void deve_retornar_200_quando_houver_gabarito() {
        get(url(port,"/simulados/MED-2018-FOR/provas/BIOLOGIA-2018/gabarito"))
                .then()
                .statusCode(OK.value());
    }

    @Test
    public void deve_retornar_404_quando_nao_houver_simulado() {
        get(url(port,"/simulados/FAR-2019-SP/provas/BIOLOGIA-2018/gabarito/"))
                .then()
                .statusCode(NOT_FOUND.value());
    }

    @Test
    public void deve_retornar_404_quando_nao_houver_prova() {
        get(url(port,"/simulados/MED-2018-FOR/provas/GEOGRAFIA-2018/gabarito/"))
                .then()
                .statusCode(NOT_FOUND.value());
    }
    ...
    @Test
    public void deve_trazer_apenas_a_escolha_certa() {
        get(url(port,"/simulados/MED-2018-FOR/provas/BIOLOGIA-2018/gabarito"))
                .then()
                .body("questoes[0].escolhas", hasSize(1));
    }

    @Test
    public void o_campo_referencia_deve_trazer_a_referencia_da_escolha_certa() {
        get(url(port,"/simulados/MED-2018-FOR/provas/BIOLOGIA-2018/gabarito"))
                .then()
                .body("questoes[0].escolhas[0].referencia", equalTo("B)"));
    }
